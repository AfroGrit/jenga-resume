...
class LLMHandler:
    ...
    def calculate_openai_cost(self, model, tokens):
        ...
        Args:
            ...
            tokens (CompletionUsage or int): Token usage statistics object.
        ...
        if model == "gpt-4o-mini":
            # Ensure tokens is an object with prompt_tokens and completion_tokens attributes
            if hasattr(tokens, 'prompt_tokens') and hasattr(tokens, 'completion_tokens'):
                openai_cost = (
                    tokens.prompt_tokens * 0.00015 + tokens.completion_tokens * 0.0006
                ) / 1000
            else:
                print("Token usage data unavailable, setting cost to 0.")
        return openai_cost
...
class RAGSystem:
    ...
    def rag(self, query, model="gpt-4o-mini"):
        ...
        "prompt_tokens": getattr(token_stats, "prompt_tokens", 0),
        "completion_tokens": getattr(token_stats, "completion_tokens", 0),
        "total_tokens": getattr(token_stats, "total_tokens", token_stats if isinstance(token_stats, int) else 0),
        "eval_prompt_tokens": getattr(rel_token_stats, "prompt_tokens", 0) if not isinstance(rel_token_stats, int) else 0,
        "eval_completion_tokens": getattr(rel_token_stats, "completion_tokens", 0) if not isinstance(rel_token_stats, int) else 0,
        "eval_total_tokens": getattr(rel_token_stats, "total_tokens", rel_token_stats if isinstance(rel_token_stats, int) else 0),
